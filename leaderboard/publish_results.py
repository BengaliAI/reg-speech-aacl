#!/usr/bin/env python3
"""Publish a scored run to the public results dataset.

Reads a private metrics.json (from score.py) and upserts one row into
bengaliAI/ben10-asr-results results.csv. Only score fields leave the
machine — never hyps, gold, or the env receipt.
"""
...
RESULTS_REPO = "bengaliAI/ben10-asr-results"
COLUMNS = [
    "rank",
    "model_id",
    "model_url",
...
REQUIRED = ["model_id", "wer", "wer_by_region", "evaluated_at"]
...
def row_from_metrics(m: dict) -> dict:
    ...
    return {
        ...
        "wer": f"{float(m['wer']):.6f}",
        "wer_by_region": json.dumps(
            m["wer_by_region"], ensure_ascii=False, separators=(",", ":")
        ),
...
def render(rows: list[dict]) -> str:
    rows = sorted(rows, key=lambda r: float(r["wer"]))
...
def main() -> None:
    ...
    if dupes and not args.replace:
        raise SystemExit(
            f"{new['model_id']} already on the board (wer={dupes[0]['wer']}); "
    ...
    HfApi().upload_file(
        ...
        repo_id=args.repo,
        repo_type="dataset",
        commit_message=f"Add score for {new['model_id']} (wer={new['wer']})",
    )
