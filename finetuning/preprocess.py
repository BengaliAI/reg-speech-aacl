...
class Config:
    ...
    id_column_name: str = field(
        default="id",
        metadata={
            "help": "The name of the dataset column containing the id of data. Defaults to 'id'"
        },
    )
...
def main():
    ...
    def add_mp3_path(examples):
        # return {
        #     "audio": [str(data_dir / f"train_mp3s/{id_}.mp3") for id_ in examples[cfg.id_column_name]]
        # }
        temp= {
            "audio": [str(data_dir / f"16_kHz_train_audio/{id_}") for id_ in examples[cfg.id_column_name]]
        }
        return temp
    ...
    # max_input_length
    print('samples train/test', len(vectorized_datasets['train']),len(vectorized_datasets['validation']))
    ...
    def is_labels_in_length_range(labels):
        return cfg.min_target_length < len(labels) < cfg.max_target_length
    ...
    print('samples after filtering label length', len(vectorized_datasets['train']),len(vectorized_datasets['validation']))
    vectorized_datasets = vectorized_datasets.filter(
        is_audio_in_length_range,
        num_proc=num_workers,
        input_columns=["input_length"],
    )
    print('samples after filtering audio length', len(vectorized_datasets['train']),len(vectorized_datasets['validation']))
    ...
    #         input_columns=["labels"],
    #     )
    # print('samples after filtering by min-max length', len(vectorized_datasets['train']),len(vectorized_datasets['validation']))
    ...
    if cfg.preprocessing_only:
        ...
        save_chunks(
            vectorized_datasets["train"], 1000, f"{training_args.output_dir}/train"
        )
        save_chunks(
            vectorized_datasets["validation"], 1000, f"{training_args.output_dir}/eval"
        )
        return
